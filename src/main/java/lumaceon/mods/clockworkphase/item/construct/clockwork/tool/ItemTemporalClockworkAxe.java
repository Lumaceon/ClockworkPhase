...
public class ItemTemporalClockworkAxe extends ItemClockworkAxe implements IKeybindAbility, ITemporalChange
{
    public ItemTemporalClockworkAxe(ToolMaterial mat)
    {
        super(mat);
    ...
    public void onUpdate(ItemStack is, World world, Entity entity, int p_77663_4_, boolean p_77663_5_)
    {
        if(entity instanceof EntityPlayer)
        {
            ...
            if(timeSand < MechanicTweaker.TIME_SAND_PER_BLOCK_BREAK_AXE && !player.inventory.getStackInSlot(player.inventory.currentItem).isEmpty())
            {
    ...
    {
        if(ValidBlockLists.BlockLogs.isBlockValidWood(world.getBlockState(pos).getBlock()))
        {
            if(world.isBlockModifiable(player, pos))
            {
                int amountToRemove = MechanicTweaker.TIME_SAND_COST_TEMPORAL_AXE;
                if(this.getTimeSandFromInventory(player.inventory) >= amountToRemove || this.getTimeSand(player.getHeldItem(hand)) >= amountToRemove)
                {
                    ...
                    world.setBlockState(pos, ModBlocks.woodTemporal.getDefaultState());
    ...
    public boolean onBlockDestroyed(ItemStack is, World world, IBlockState state, BlockPos pos, EntityLivingBase entity)
    {
        ...
        if(entity instanceof EntityPlayer)
        {
            ...
            int amountToRemove = MechanicTweaker.TIME_SAND_PER_BLOCK_BREAK_AXE;
            EntityPlayer player = (EntityPlayer)entity;
            amountToRemove -= this.removeTimeSandFromInventory(player.inventory, amountToRemove);
