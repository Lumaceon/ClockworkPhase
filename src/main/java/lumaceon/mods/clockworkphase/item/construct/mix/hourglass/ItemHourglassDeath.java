    ...
    public void onUpdate(ItemStack is, World world, Entity entity, int p_77663_4_, boolean p_77663_5_)
    {
        if(entity instanceof EntityPlayer && NBTHelper.getBoolean(is, NBTTags.ACTIVE))
        {
            ...
            if (speed > 10)
            {
                ...
                {
                    player.addPotionEffect(new PotionEffect(Potion.hunger.getId(), 20, speed / 150, false));
                }
                if (!player.isPotionActive(Potion.poison))
                {
                    player.addPotionEffect(new PotionEffect(Potion.poison.getId(), 20, speed / 300, false));
                }
