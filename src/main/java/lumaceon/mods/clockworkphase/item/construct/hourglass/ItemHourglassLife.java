...
public class ItemHourglassLife extends ItemHourglass
{
    @Override
    public void onUpdate(ItemStack is, World world, Entity entity, int p_77663_4_, boolean p_77663_5_)
    {
        if(entity instanceof EntityPlayer && NBTHelper.getBoolean(is, NBTTags.ACTIVE))
        {
            ...
            int tensionCost = (int)Math.round(MechanicTweaker.LIFE_HOURGLASS_TENSION_COST * Math.pow(efficiency, 2));
            if(PhaseHelper.getPhaseForWorld(world).equals(Phases.LIFE)) { tensionCost *= 0.1; }
            ...
            if(speed > 10)
            {
                ...
                player.addPotionEffect(new PotionEffect(MobEffects.REGENERATION, 100, speed / 150, false, true));
                this.removeTension(is, tensionCost);
            }
        }
    }
