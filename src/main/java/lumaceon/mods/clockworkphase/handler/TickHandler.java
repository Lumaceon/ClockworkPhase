...
public class TickHandler
{
    ...
    public void onWorldTick(TickEvent.WorldTickEvent event)
    {
        if(event.world.getWorldTime() % 600 == 0) //Once every 30 seconds.
        {
            ...
            {
                ExtractorAreas area = (ExtractorAreas)ExtractorAreas.EXTRACTORS[n].get(event.world.provider.getDimension()); //Gets the areas for the world.
                if(area != null)
                {
                    for(int g = 0; g < area.areas.size(); g++) //For each area.
                    {
                        ...
                        {
                            te = event.world.getTileEntity(new BlockPos(area.areas.get(g).extractorX, area.areas.get(g).extractorY, area.areas.get(g).extractorZ));
